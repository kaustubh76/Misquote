...
# The pruned-state hook used to live here. It now lives in `tests/conftest.py`,
# because `pytest_runtest_makereport` in a package conftest only applies to that
# package — and `tests/registry/test_termix_escrow_fork.py` forks the same way
# from the same free endpoints, so it was failing outright where these tests
# skipped. One implementation, per the rule the drivers taught us.
