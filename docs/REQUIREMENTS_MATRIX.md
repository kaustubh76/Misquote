...
| `studio.bnbchain.org/install` | **Dead — DNS does not resolve**, re-checked 2 Sep 2026 (NXDOMAIN), and the npm package's declared repository 404s. **Neither blocks the CLI**: `@bnbagent/studio-cli` is on npm at 0.0.13 across 39 versions, installs, and its `bag` binary offers `erc8004`, `erc8183` and `x402` subcommands. Probe recorded in `vetting/identity/studio-probe.json`. Two rival CLIs both called `bag`; the PyPI `bnbagent-studio` is stale. |
...
| ERC-8183 hire call invoked from an external script | **RESOLVED — and the recorded blocker was the wrong one.** This row said no job could be read back because `nextJobId()`, `jobCount()` and `jobs(uint256)` revert. They do, and it did not matter: the deployment's accessors are `jobCounter()` and `getJob(uint256)`, so the item was blocked on the names we had guessed rather than on the contract. `scripts/hire_agent.py` mined `approve`, `createJob` and `setBudget` on chapel (job 746); `scripts/hire_mainnet.py` funded **job 56681 on BSC mainnet** with 0.1 of the payment token and `scripts/claim_refund.py` brought it back, then job 56718 was funded and submitted. What is still open is `settle` alone, which reverts `NotDecided()` until the 7-day window runs — tracked by `check_escrow_flow` in the go/no-go rather than by this table. `Readme.md` §8 ticks this row and the two now agree. |
