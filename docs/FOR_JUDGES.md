...
```bash
make setup                    # uv sync
make test                     # 2,270 tests, no network, ~35s
...
**2314 tests: 2270 offline, 44 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **323,233** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 323,371 (+138) |
