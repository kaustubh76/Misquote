...
```bash
make setup                    # uv sync
make test                     # 2,416 tests, no network, ~13 min
...
**2469 tests: 2416 offline, 53 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **336,492** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 336,629 (+137) |
