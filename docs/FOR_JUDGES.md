...
```bash
make setup                    # uv sync
make test                     # 2,517 tests, no network, ~13 min
...
**2570 tests: 2517 offline, 53 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **338,535** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 338,672 (+137) |
