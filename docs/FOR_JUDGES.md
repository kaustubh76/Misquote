...
```bash
make setup                    # uv sync
make test                     # 2,370 tests, no network, ~35s
...
**2422 tests: 2370 offline, 52 against a live chain or a fork.**
...
|---|---|---|---|---|---|
| Earn | `chain` | 1 | 8.82 – 16.35% | 18.36 – 35.55% | **+16.80pp** |
| Market-make | `chain` | 1 | 8.82 – 16.35% | 20.73 – 32.07% | **+12.71pp** |
| Protect | `chain` | 1 | 9.38 – 14.94% | -55.19 – -52.88% | **-68.59pp** |
| Choose | `chain` | 0.03181 | 5.98 – 16.77% | 5.98 – 16.77% | **+0.00pp** |
...
- **The Agent Studio deployment.** It is the first entry on the not-built ledger
  ...
  What is actually open is a custody decision: deploying hands a wallet key to
  the vendor's infrastructure. That is a choice about money, not a missing
  afternoon, and it has not been made.
...
| | |
|---|---|
| agents TermiX's explorer indexes | **329,013** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 329,152 (+139) |
