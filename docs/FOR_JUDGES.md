...
```bash
make setup                    # uv sync
make test                     # 2,658 tests, no network, ~13 min
...
**2711 tests: 2658 offline, 53 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **341,935** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 342,073 (+138) |
