...
```bash
make setup                    # uv sync
make test                     # 2,378 tests, no network, ~13 min
...
**2430 tests: 2378 offline, 52 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **332,963** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 333,100 (+137) |
