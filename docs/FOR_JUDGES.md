...
```bash
make setup                    # uv sync
make test                     # 2,450 tests, no network, ~13 min
...
**2503 tests: 2450 offline, 53 against a live chain or a fork.**
...
- **Sentinel loses to the passive baseline, and we know why.**
  `docs/AGENT_ADVANTAGE.md` reports −68.59pp on Protect. Earn and Market-make
  now come out ahead — +16.80pp and +12.71pp, bands separated — on a report
...
| | |
|---|---|
| agents TermiX's explorer indexes | **338,179** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 338,316 (+137) |
