...
```bash
make setup                    # uv sync
make test                     # 2,639 tests, no network, ~13 min
...
| …and that Solidity is PancakeSwap's | The corpus above is generated against **Uniswap's** libraries. All six that `Exposer.sol` imports are token-identical to PancakeSwap's own copies at a pinned commit — same licence header changed, same lines rewrapped, same arithmetic. This step used to be a comment | `make fork-parity`, recorded in `vetting/runs/fork-parity.json` |
...
**2692 tests: 2639 offline, 53 against a live chain or a fork.**
...
Two tasks sharing one baseline and one agent would be one task relabelled, so a
test asserts no pair repeats — Earn and Market-make deliberately share a control,
because measuring Warden and Grid against the *same* passive position is what a
control is for, and replaying it twice would burn an hour and a half to produce
two identical columns. Task 3's baseline is an ablation rather than a different
strategy, so the comparison isolates the withdrawal decision and nothing else.

**Two of the five now go the agent's way, and both have non-overlapping bands.**
That is a change of result, not of wording: the previous figures were generated
on 23 August and the freshness gate had been reporting them as fifty engine
commits behind. Every fix since — the capital-basis default, the sigma
estimator, the LVR clamp — moved them. The headline "our agents lose" was
substantially an artifact of a report nobody had regenerated.

The table is generated from `advantage.json` rather than typed, so it cannot
...
|---|---|---|---|---|---|
| Earn | `chain` | 1 | 8.94 – 16.35% | 20.70 – 36.15% | **+17.21pp** |
...
**What it says is that hiring the agent loses one task badly** — Protect,
against a position minted once and never touched. It said "loses on the
liquidity tasks", which was true when it was written and stopped being true
when Earn and Market-make turned; the sentence sat two hundred lines under
"Two of the five now go the agent's way" and contradicted it. The numbers are
in the table above, and so is the reason — the report derives it from its own
columns rather than asserting it, and the worst task's figures are reproduced
here:

<!-- derived:mechanism — regenerated by `make judges`; do not hand-edit -->
```
    MINT 596 · PULL 595 · RECENTRE 0
    19.2 pull-and-re-mint cycles per calendar day, against max_rebalances_per_day
    in range 87.0% of the window; gas alone 63% of capital annualised
```
...
There are two, and the first is the one to click.

**[`/simulate`](../apps/web/src/app/simulate/view.tsx) — set up a position and
see what it would have paid.** Pick a pool, a range half-width, an amount and a
window; every figure is a replay of swaps already on chain, so it signs nothing
and cannot cost anybody anything. 1,400 positions were replayed by
`scripts/simulate_report.py` — seven widths, twenty rolling windows, five sizes,
two pools — and the page selects among them rather than computing anything.

Two things it refuses to do, and both were found by building it:

- **It will not multiply.** The obvious design was one sweep and a number box,
  since APR is per unit of capital. Measured, ten times the capital earns
  **9.988 times** the fees and a *lower* rate: `LvrAccountant` prorates each
  swap by liquidity share, so a bigger position dilutes itself. A multiplication
  would have deleted that in the direction that flatters the larger position, so
  every size offered is a size the engine replayed.
- **It will not clamp.** A1 caps a replayed position at 1% of the venue's depth.
  On the flagship that is 4.21 WBNB at ±80 ticks; on the 0.25% pool it is
  **0.026 WBNB**, so most of the ladder is refused there — and widening the
  range to ±800 raises it to 0.25 and lets the next size in. That is a capacity
  finding an LP cannot get from a fee APR, and the refusal names the depth it
  came from.

**[`/venue`](../apps/web/src/app/venue/view.tsx) — the integration surface
underneath it**, generated from `scripts/venue_report.py`.
...
The half that is *not* a divergence is the larger half, and it was the one thing
here taken on trust: the differential corpus is generated against Uniswap's
Solidity, so it says nothing about PancakeSwap unless the fork left those
libraries alone. That clause lived in two code comments until `make fork-parity`
made it a reading — six libraries, token-identical, at a pinned commit.
...
- **Five of the nine badge checks have no executable proof-of-concept.** Four
  ...
  reports 18 decimals or it does not — and no transaction demonstrates a
...
- **Sentinel loses to the passive baseline, and we know why.**
  `docs/AGENT_ADVANTAGE.md` reports the loss on Protect, and Earn and
  Market-make now come out ahead with their bands separated. The figures were
  typed here and drifted from the report by hundredths, which is small and is
  exactly the failure this document's `derived:` blocks exist to prevent: they
  sat outside one, so `check_docs_current` could not see them. The numbers live
  in the `derived:advantage` table above and are not repeated here — on a report
  regenerated against the current engine; the figures this paragraph used to
  quote were fifty engine commits stale. Protect's loss is real and is not being
  softened: it cuts realized convexity cost 1.8x and pays more than that for the
  withdrawals.
...
| | |
|---|---|
| agents TermiX's explorer indexes | **341,374** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 341,511 (+137) |
