...
```bash
make setup                    # uv sync
make test                     # 2,277 tests, no network, ~35s
...
**2325 tests: 2277 offline, 48 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **325,587** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 325,728 (+141) |
