...
```bash
make setup                    # uv sync
make test                     # 2,638 tests, no network, ~13 min
...
**2691 tests: 2638 offline, 53 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **341,244** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 341,381 (+137) |
