...
```bash
make setup                    # uv sync
make test                     # 2,275 tests, no network, ~35s
...
**2319 tests: 2275 offline, 44 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **324,773** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 324,912 (+139) |
