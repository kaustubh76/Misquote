...
```bash
make setup                    # uv sync
make test                     # 2,453 tests, no network, ~13 min
...
**2506 tests: 2453 offline, 53 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **338,181** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 338,319 (+138) |
