...
```bash
make setup                    # uv sync
make test                     # 2,358 tests, no network, ~35s
...
**2410 tests: 2358 offline, 52 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **328,733** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 328,904 (+171) |
