...
```bash
make setup                    # uv sync
make test                     # 2,284 tests, no network, ~35s
...
**2336 tests: 2284 offline, 52 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **325,990** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 326,127 (+137) |
