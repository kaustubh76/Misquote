...
```bash
make setup                    # uv sync
make test                     # 2,651 tests, no network, ~13 min
...
**2704 tests: 2651 offline, 53 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **341,762** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 341,899 (+137) |
