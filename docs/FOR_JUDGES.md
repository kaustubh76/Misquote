...
```bash
make setup                    # uv sync
make test                     # 2,546 tests, no network, ~13 min
...
**2599 tests: 2546 offline, 53 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **340,811** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 340,948 (+137) |
