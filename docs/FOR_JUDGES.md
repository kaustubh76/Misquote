...
```bash
make setup                    # uv sync
make test                     # 2,346 tests, no network, ~35s
...
**2398 tests: 2346 offline, 52 against a live chain or a fork.**
...
| | |
|---|---|
| agents TermiX's explorer indexes | **326,366** |
| endpoint | `/api/v1/explorer/agents`, public, no credentials |
| the registry's own high-water id, read in the same build | 326,503 (+137) |
