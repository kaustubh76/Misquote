...
either trustworthy and which constant is right. `tests/estimators/test_apr.py` replays 172 real
...
So the numbers are no longer here. `docs/FOR_JUDGES.md` carries them in a block
regenerated by `make judges` from the card itself, and the card is
`apps/web/public/artifacts/router.json`. What this assumption is *about* does
...
Published for the same reason the table above is: `router.json` renders every one of these, and
...
On that sample `sigma` is not an estimate. `estimators/sigma.py::shrink` returns the
...
quoting something that does not exist. `estimators/pool_apr.py` therefore cannot be
...
deploys. The fee arithmetic is `lvr/accountant.py`'s — reused rather than restated,
...
**Added.** `scripts/pools_report.py` answers *which pool, how wide* — and the shape of
that answer is constrained twice over.

*Bands, not a leaderboard.* Every figure is a **P25–P75 range over rolling windows**
carrying its observation count (A5), using `replay/ranges.py`'s own `MIN_SAMPLES` and
...
this sample size"* — the sentence `tearsheet/advantage.py` already uses — rather than
